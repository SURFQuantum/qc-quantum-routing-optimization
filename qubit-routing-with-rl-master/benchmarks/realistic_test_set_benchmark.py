...
if __name__ == '__main__':
    ...
    print(f' p value after mapping: {p} ')
    results = p.starmap(perform_run, inputs)

    print(f' results value in starmap before for loop: {results}')
    results.sort(key=lambda r: r[0])

    print(f' results value after sorting: {results}')
    print()
    for r in results:
        print(f'results of starmap= {r}')
    print()
